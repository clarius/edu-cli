...
{
    internal class AddUsersCommand : CommandBase
    {
        internal AddUsersCommand(string[] args) : base(args) { }
        ...
        async public override Task RunInternal()
        {
            ...
            if (errorCount > 0)
            {
                Log.Logger.Error("Please fix the above errors and run this command again");
                return;
            }
        ...
        {
            return new List<string>() { "--AddUsers", "-au" };
        }

        public override bool UseCache => true;

        public override string Name => "Add Users";
        public override string Description => "Adds one or more users from a .csv file";
    }
}
